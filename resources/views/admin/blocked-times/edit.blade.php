...
<div class="p-6">
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-900">Edit Blocked Time</h1>
            <div class="flex space-x-3">
                <a href="{{ route('admin.blocked-times.show', $blockedTime) }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded">
                    View Details
                </a>
        ...
        <div class="bg-white shadow rounded-lg">
            <form method="POST" action="{{ route('admin.blocked-times.update', $blockedTime) }}" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $blockedTime->title) }}" 
                    ...
                    <div>
                        ...
                        <input type="date" name="blocked_date" id="blocked_date" value="{{ old('blocked_date', $blockedTime->blocked_date->format('Y-m-d')) }}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" 
                               required>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="block_type" id="block_type" 
                            ...
                            @foreach($blockTypes as $key => $label)
                                <option value="{{ $key }}" {{ old('block_type', $blockedTime->block_type) == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="md:col-span-2">
                        <label class="flex items-center">
                            <input type="checkbox" name="is_all_day" id="is_all_day" value="1" 
                                   {{ old('is_all_day', $blockedTime->is_all_day) ? 'checked' : '' }}
                    ...
                    <div id="time-fields" class="grid grid-cols-2 gap-4 md:col-span-2">
                        <div>
                            ...
                            <input type="time" name="start_time" id="start_time" 
                                   value="{{ old('start_time', $blockedTime->start_time ? \Carbon\Carbon::parse($blockedTime->start_time)->format('H:i') : '') }}" 
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
                            <label for="end_time" class="block text-sm font-medium text-gray-700 mb-2">End Time</label>
                            <input type="time" name="end_time" id="end_time" 
                                   value="{{ old('end_time', $blockedTime->end_time ? \Carbon\Carbon::parse($blockedTime->end_time)->format('H:i') : '') }}" 
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="recurrence_type" id="recurrence_type" 
                            ...
                            @foreach($recurrenceTypes as $key => $label)
                                <option value="{{ $key }}" {{ old('recurrence_type', $blockedTime->recurrence_type) == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div id="recurrence-end-field">
                        ...
                        <input type="date" name="recurrence_end_date" id="recurrence_end_date" 
                               value="{{ old('recurrence_end_date', $blockedTime->recurrence_end_date?->format('Y-m-d')) }}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                            ...
                            @foreach($enquiryTypes as $key => $label)
                                <label class="flex items-center">
                                    <input type="checkbox" name="blocked_enquiry_types[]" value="{{ $key }}" 
                                           {{ in_array($key, old('blocked_enquiry_types', $blockedTime->blocked_enquiry_types ?? [])) ? 'checked' : '' }}
                                           class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200">
                    ...
                    <div class="md:col-span-2">
                        <label class="flex items-center">
                            <input type="checkbox" name="is_active" value="1" 
                                   {{ old('is_active', $blockedTime->is_active) ? 'checked' : '' }}
                                   class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200">
                ...
                <div class="flex justify-end space-x-3 pt-6 border-t">
                    <a href="{{ route('admin.blocked-times.show', $blockedTime) }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-6 py-2 rounded">
                        Cancel
                    </a>
